/* jQuery Bracket | Copyright (c) Teijo Laine 2011-2013 | Licenced under the MIT licence */ ! function (a) {
    ...
    var v = function (e) {
        ...
        function i(c, d, i, j, k) {
            function l(c, d, i) {
                ...
                var n = d.name ? d.name : "--",
                    ...
                    q = a('<div class="labelEquipo"></div>').appendTo(p);
                return 0 === c && p.attr("data-resultid", "team-" + k), e.decorator.render(q, n, j), b(d.idx) && p.attr("data-teamid", d.idx), null === d.name ? p.addClass("na") : g(o).name === d.name ? p.addClass("win") : h(o).name === d.name && p.addClass("lose"), p.append(l), null !== d.name && i && e.save && e.save && (q.addClass("editable"), q.click(function () {
                    function b() {
                        function h(h, i) {
                            ...
                            var j = e.el.find(".team[data-teamid=" + (d.idx + 1) + "] div.labelEquipo:first");
        ...
        var E = 80 * r.teams.length;
        C.css("height", E), A && r.teams.length <= 2 && !e.skipConsolationRound && (E += 40, y.css("height", E)), D && D.css("height", C.height() / 2);
        var F;
        //esta variable indica el ancho de cada ronda
        var anchoRonda = 225;
        return F = A ? Math.log(2 * r.teams.length) / Math.log(2) : 2 * (Math.log(2 * r.teams.length) / Math.log(2) - 1) + 1, y.css("width", anchoRonda * F + 10), v = s(C, z && z[0] ? z[0] : null, i), A || (w = s(D, z && z[1] ? z[1] : null, i), x = s(B, z && z[2] ? z[2] : null, i)), o(v, r.teams, A, e.skipConsolationRound), A || (p(v, w, r.teams.length), q(x, v, w, e.skipSecondaryFinal, e.skipConsolationRound, y)), f(!1), {
            data: function () {
                return e.init
            }
        }
